...
export default function GlobalSyncScreen({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    const checkSyncStatus = async () => {
      try {
        const res = await fetch('http://localhost:5000/api/auth/meta/sync-status');
        if (res.ok) {
          ...
          if (data.isSyncing && initialJobs === null && data.jobsRemaining > 0) {
  ...
  if (isSyncing) {
    const estimatedSeconds = jobsRemaining * 18;
    ...
    const progress = initialJobs && initialJobs >= jobsRemaining 
      ? Math.max(5, Math.round(((initialJobs - jobsRemaining) / initialJobs) * 100)) 
      : 15;
        ...
        <div className="bg-white rounded-[24px] shadow-2xl border border-gray-100 w-full max-w-[420px] overflow-hidden animate-in zoom-in-95 duration-500">
          ...
          <div className="bg-slate-50 border-b border-gray-100 p-8 flex flex-col items-center justify-center relative overflow-hidden">
            ...
            <div className="absolute top-0 left-0 w-full h-full overflow-hidden opacity-60">
               ...
               <div className="absolute -bottom-12 -right-12 w-48 h-48 bg-indigo-200 rounded-full mix-blend-multiply filter blur-3xl opacity-70 animate-pulse" style={{ animationDelay: '1s' }}></div>
            </div>
          ...
          {/* Content Area */}
          <div className="p-8">
            <h2 className="text-xl font-bold text-gray-900 text-center mb-2">Syncing your workspace</h2>
            <p className="text-sm text-gray-500 text-center mb-8 leading-relaxed">
              We're securely downloading your historical data from Meta. You can safely leave this page, but keeping it open ensures the fastest sync.
            </p>

            {/* Progress Section */}
            <div className="space-y-4">
              <div className="flex justify-between items-end mb-2">
                <span className="text-xs font-bold text-gray-900 uppercase tracking-wider">Sync Progress</span>
              ...
              <div className="grid grid-cols-2 gap-3 pt-4">
                <div className="bg-gray-50 rounded-xl p-3.5 border border-gray-100 flex flex-col items-center justify-center text-center shadow-sm">
                  <p className="text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-1">Items Remaining</p>
                  <p className="text-xl font-black text-gray-900">{jobsRemaining}</p>
                </div>
                <div className="bg-gray-50 rounded-xl p-3.5 border border-gray-100 flex flex-col items-center justify-center text-center shadow-sm">
                  <p className="text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-1">Est. Time</p>
                  <p className="text-xl font-black text-gray-900">~{etaMinutes}m</p>
                </div>
              </div>
            </div>
