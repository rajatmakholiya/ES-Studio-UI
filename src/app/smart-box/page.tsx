...
const inboxMessages = [
  {
    ...
    preview: 'Absolutely loving the new update! The analytics are so much clearer now. Great job team 🚀',
    isUnread: true,
  ...
  },
  {
    ...
    preview: 'This is exactly the breakdown we needed for our Q3 reporting. Will there be an export to PDF feature soon?',
    isUnread: true,
  ...
  },
  {
    ...
    preview: 'Hi! I saw your recent post about the premium plan. Can you send me the pricing details for an agency of 10?',
    isUnread: false,
  ...
  },
  {
    ...
    preview: 'Thanks for sharing this guide. Found the section on audience retention especially helpful.',
    isUnread: false,
    platformColor: 'text-blue-500 bg-blue-50'
  }
];
...
export default function SmartBoxPage() {
  ...
  const activeMessage = inboxMessages.find(m => m.id === activeMessageId) || inboxMessages[0];
    ...
    <div className="flex h-[calc(100vh-8rem)] flex-col gap-6">
      ...
      <div className="flex items-end justify-between shrink-0">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">Manage all cross-channel conversations in one unified inbox.</p>
        </div>
        ...
        <div className="flex w-1/3 flex-col rounded-2xl border border-gray-100 bg-white shadow-[0_2px_10px_rgba(0,0,0,0.02)] overflow-hidden">
          ...
          <div className="border-b border-gray-100 p-4 shrink-0">
            ...
            <div className="mt-4 flex gap-4 text-[11px] font-bold uppercase tracking-wider text-gray-500">
              <button className="text-[#6366f1] border-b-2 border-[#6366f1] pb-1">Needs Action (2)</button>
            ...
            {inboxMessages.map((msg) => (
              ...
              >
                <div className="relative shrink-0">
                  <img src={msg.avatar} alt="" className="h-10 w-10 rounded-full object-cover" />
                  <div className={`absolute -bottom-1 -right-1 flex h-5 w-5 items-center justify-center rounded-full border-2 border-white ${msg.platformColor}`}>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between mb-1">
                    <p className={`text-sm truncate ${msg.isUnread ? 'font-bold text-gray-900' : 'font-semibold text-gray-700'}`}>
                      {msg.sender}
                    </p>
                  ...
                  <p className="text-xs text-gray-500 truncate mb-1">{msg.type} • {msg.handle}</p>
                  <p className={`text-sm line-clamp-2 ${msg.isUnread ? 'text-gray-800 font-medium' : 'text-gray-500'}`}>
                    {msg.preview}
                  </p>
                </div>
        ...
        <div className="flex flex-1 flex-col rounded-2xl border border-gray-100 bg-white shadow-[0_2px_10px_rgba(0,0,0,0.02)] overflow-hidden">
          ...
          <div className="flex items-center justify-between border-b border-gray-100 p-6 shrink-0">
            <div className="flex items-center gap-4">
              <img src={activeMessage.avatar} alt="" className="h-12 w-12 rounded-full object-cover" />
              <div>
                <h2 className="text-base font-bold text-gray-900">{activeMessage.sender}</h2>
                <div className="flex items-center gap-2 mt-0.5">
                  <span className={`flex items-center gap-1 rounded-full px-2 py-0.5 text-[10px] font-bold ${activeMessage.platformColor}`}>
                    {getPlatformIcon(activeMessage.platform)}
                    {activeMessage.platform}
                  </span>
                  <span className="text-xs font-medium text-gray-500">{activeMessage.handle}</span>
                </div>
              </div>
            </div>
          ...
          <div className="flex-1 overflow-y-auto p-6 bg-[#fcfcfd]">
            <div className="flex flex-col gap-6">
              ...
              <div className="flex items-start gap-4">
                <img src={activeMessage.avatar} alt="" className="h-8 w-8 rounded-full object-cover shrink-0" />
                <div className="flex flex-col gap-1 max-w-[80%]">
                  <div className="flex items-baseline gap-2">
                    <span className="text-sm font-bold text-gray-900">{activeMessage.sender}</span>
                    <span className="text-xs text-gray-500">{activeMessage.type}</span>
                  </div>
            ...
            <div className="rounded-xl border border-gray-200 bg-gray-50 focus-within:border-[#6366f1] focus-within:bg-white focus-within:ring-1 focus-within:ring-[#6366f1] transition-all">
              ...
              <div className="flex items-center justify-between border-t border-gray-100 p-2">
                <div className="flex gap-2 px-2">
                  <button className="text-[10px] font-bold uppercase tracking-wider text-gray-400 hover:text-gray-900">Add Internal Note</button>
                </div>
