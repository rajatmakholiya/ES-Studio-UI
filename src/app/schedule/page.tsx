...
const calendarDays = [
  ...
  { 
    ...
    isCurrentMonth: true, 
    posts: [
      { id: 1, title: 'Product Launch', type: 'instagram', color: 'bg-pink-50 text-pink-600', icon: Instagram }
    ] 
  },
  ...
  { 
    date: '5', 
    isCurrentMonth: true, 
    isActive: true, // Highlights the specific active day from your mockup
    posts: [
      { id: 2, title: 'Demo Reel', type: 'youtube', color: 'bg-red-50 text-red-600', icon: Youtube },
      { id: 3, title: 'Blog Update', type: 'web', color: 'bg-indigo-50 text-indigo-600', icon: FileText }
    ] 
  },
  ...
  { 
    ...
    isCurrentMonth: true, 
    posts: [
      { id: 4, title: 'Hiring Post', type: 'linkedin', color: 'bg-blue-50 text-blue-600', icon: Briefcase }
    ] 
  },
  ...
  { 
    ...
    isCurrentMonth: true, 
    posts: [
      { id: 5, title: 'Trendy Sound', type: 'tiktok', color: 'bg-gray-100 text-gray-700', icon: Music }
    ] 
  },
  ...
  { 
    ...
    isCurrentMonth: true, 
    posts: [
      { id: 6, title: 'Lifestyle Reel', type: 'instagram', color: 'bg-pink-50 text-pink-600', icon: Instagram }
    ] 
  },
...
export default function SchedulePage() {
  return (
    <div className="flex flex-col gap-6 lg:flex-row">
      ...
      <div className="flex-1 space-y-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center rounded-full bg-gray-100 p-1">
            <button className="rounded-full bg-white px-4 py-1.5 text-xs font-semibold shadow-sm text-gray-900">Month</button>
            <button className="rounded-full px-4 py-1.5 text-xs font-semibold text-gray-500 hover:text-gray-900">Week</button>
            <button className="rounded-full px-4 py-1.5 text-xs font-semibold text-gray-500 hover:text-gray-900">Day</button>
          </div>
        </div>
        ...
        <div className="flex items-end justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-500">12 posts scheduled for this month</p>
          </div>
        ...
        <div className="mt-8">
          ...
          <div className="grid grid-cols-7 gap-4 mb-4 text-center">
            {['MON', 'TUE', 'WED', 'THU', 'FRI', 'SAT', 'SUN'].map((day) => (
              <div key={day} className="text-[10px] font-bold uppercase tracking-widest text-gray-400">
                {day}
              </div>
            ))}
          </div>
          ...
          <div className="grid grid-cols-7 gap-4">
            {calendarDays.map((day, index) => (
              ...
              >
                ...
                <span className={`text-xs font-bold ${day.isCurrentMonth ? 'text-gray-900' : 'text-gray-300'}`}>
                  {day.date}
                </span>
      ...
      {/* RIGHT COLUMN: Sidebar (Upcoming, Drafts, Premium) */}
      <div className="w-full lg:w-80 space-y-8 pt-16">
        
        {/* Upcoming List */}
        <div>
          <div className="mb-4 flex items-center justify-between">
            <h3 className="text-[11px] font-bold uppercase tracking-wider text-gray-400">Upcoming List</h3>
            <button className="text-[11px] font-bold text-[#6366f1] hover:underline">View All</button>
          </div>
          <div className="space-y-4">
            {[
              { title: 'Autumn Sale Banner', time: 'Tomorrow, 10:00 AM', color: 'bg-emerald-400' },
              { title: 'Product Feature Walkthrough', time: 'Oct 7, 02:30 PM', color: 'bg-red-400' }
            ].map((item, i) => (
              <div key={i} className="flex gap-3">
                <div className="h-10 w-10 flex-shrink-0 rounded-lg bg-orange-100 overflow-hidden">
                   {/* Placeholder Image */}
                   <img src={`https://images.unsplash.com/photo-1611162617474-5b21e879e113?q=80&w=100&auto=format&fit=crop&sig=${i}`} alt="" className="h-full w-full object-cover" />
                </div>
                <div>
        ...
        <div>
          <div className="mb-4 flex items-center justify-between">
            <h3 className="text-[11px] font-bold uppercase tracking-wider text-gray-400">Drafts</h3>
            <span className="flex h-5 w-5 items-center justify-center rounded-full bg-indigo-50 text-[10px] font-bold text-[#6366f1]">3</span>
          </div>
          <div className="space-y-2">
            {[
              ...
              <div key={i} className="rounded-xl border border-dashed border-gray-200 bg-gray-50 p-3">
                <p className="text-sm font-semibold text-gray-900">{draft}</p>
                <p className="mt-0.5 text-[10px] italic text-gray-400">No scheduled date yet</p>
              </div>
            ))}
          </div>
        </div>
